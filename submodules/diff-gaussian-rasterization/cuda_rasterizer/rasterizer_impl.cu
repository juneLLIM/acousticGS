...
__global__ void duplicateWithKeys(
	...
	if(radii[idx] > 0) {
		...
		if(rects == nullptr)
			...
			getRect(means2D[idx], rects[idx], rect_min, rect_max, grid);

		const float2 xy = means2D[idx];
		...
		// key/value pair. The key is |  tile ID  |      distance      |,
		// and the value is the ID of the Gaussian. Sorting the values 
		// with this key yields Gaussian IDs in a list, such that they
		// are first sorted by tile and then by distance. 
...
CudaRasterizer::GeometryState CudaRasterizer::GeometryState::fromChunk(char*& chunk, size_t P) {
	GeometryState geom;
	obtain(chunk, geom.distances, P, 128);
...
CudaRasterizer::ImageState CudaRasterizer::ImageState::fromChunk(char*& chunk, size_t N) {
	...
	obtain(chunk, img.pixel_phasors, N * NUM_CHANNELS, 128);
...
std::tuple<int, int> CudaRasterizer::Rasterizer::forward(
	...
	// Run preprocessing per-Gaussian (transformation, bounding, conversion of SHs to Phasor)
#define PREPROCESS(V, RotationModel) \
		CHECK_CUDA((FORWARD::preprocess<V, RotationModel>( \
			P, D, M, W, H, \
			(glm::vec3*)micpos, \
			means5D, \
			shs, \
	...
	else if(gaussian_version == 3) {
		PREPROCESS(3, DoubleQuaternion);
	}
	else if(gaussian_version == 2) {
		PREPROCESS(2, DoubleQuaternion);
	}
	else {
		PREPROCESS(1, NoRotation);
	}
...
#undef PREPROCESS
	...
	// For each instance to be rendered, produce adequate [ tile | distance ] key 
	...
	const float* feature_ptr = geomState.phasors;
...
void CudaRasterizer::Rasterizer::backward(
	...
	// opacity and Phasor of Gaussians from per-pixel loss gradients.
	const float* phasor_ptr = geomState.phasors;
...
#define PREPROCESS(V, RotationModel) \
	CHECK_CUDA((BACKWARD::preprocess<V, RotationModel>( \
		P, D, M, W, H, \
		(glm::vec3*)micpos, \
		means5D, \
		shs, \
